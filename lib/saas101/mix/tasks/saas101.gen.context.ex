defmodule Mix.Tasks.Saas101.Gen.Context do
  ...
  Generates a context with functions around an Ecto schema.
      ...
      $ mix saas101.gen.context Accounts User users name:string age:integer
  ...
  a `--table` option may be provided. For example:
      ...
      $ mix saas101.gen.context Accounts User users --table cms_users
  ...
  Read the documentation for `saas101.gen.schema` for more information on
  attributes.
  ...
  def run(args) do
    if Mix.Project.umbrella?() do
      Mix.raise "mix saas101.gen.context must be invoked from within your *_web application root directory"
    end
  ...
  def build(args, help \\ __MODULE__) do
    ...
    schema = Mix.Tasks.Phx.Gen.Schema.build([schema_module, plural | schema_args], opts, help)
  ...
  def ensure_context_file_exists(%Context{file: file} = context, paths, binding) do
    unless Context.pre_existing?(context) do
      Mix.Generator.create_file(file, Mix.Phoenix.eval_from(paths, "priv/templates/saas101.gen.context/context.ex", binding))
    end
  end
  ...
  defp inject_schema_access(%Context{file: file} = context, paths, binding) do
    ...
    paths
    |> Mix.Phoenix.eval_from("priv/templates/saas101.gen.context/#{schema_access_template(context)}", binding)
  ...
  def ensure_test_file_exists(%Context{test_file: test_file} = context, paths, binding) do
    unless Context.pre_existing_tests?(context) do
      Mix.Generator.create_file(test_file, Mix.Phoenix.eval_from(paths, "priv/templates/saas101.gen.context/context_test.exs", binding))
    end
  end
  ...
  defp inject_tests(%Context{test_file: test_file} = context, paths, binding) do
    ...
    paths
    |> Mix.Phoenix.eval_from("priv/templates/saas101.gen.context/test_cases.exs", binding)
  ...
  def ensure_test_fixtures_file_exists(%Context{test_fixtures_file: test_fixtures_file} = context, paths, binding) do
    unless Context.pre_existing_test_fixtures?(context) do
      Mix.Generator.create_file(test_fixtures_file, Mix.Phoenix.eval_from(paths, "priv/templates/saas101.gen.context/fixtures_module.ex", binding))
    end
  end
  ...
  defp inject_test_fixture(%Context{test_fixtures_file: test_fixtures_file} = context, paths, binding) do
    ...
    paths
    |> Mix.Phoenix.eval_from("priv/templates/saas101.gen.context/fixtures.ex", binding)
  ...
  def raise_with_help(msg) do
    ...
    mix saas101.gen.html, saas101.gen.json, saas101.gen.live, and saas101.gen.context
    ...
    For example:
        ...
        mix saas101.gen.html Accounts User users name:string
        mix saas101.gen.json Accounts User users name:string
        mix saas101.gen.live Accounts User users name:string
        mix saas101.gen.context Accounts User users name:string
